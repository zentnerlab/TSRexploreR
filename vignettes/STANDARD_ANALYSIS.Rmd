...
The next step is to normalize TSS counts using the edgeR TMM method -
if the counts you input were already normalized, this step can be safely skipped.
...
The example below uses a 'GTF' file from Ensembl (R64-1-1 Release 99) and annotates each TSS to the beginning of the nearest gene.
...
```{r fig.height=3}
plot_threshold_exploration(exp, samples="S288C_WT_1", point_size=0.5) +
...
plot_sequence_logo(
  exp, threshold=10, dominant=TRUE, samples="S288C_WT_1",
